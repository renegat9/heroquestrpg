...
Six **traits d'extension** sont dans le moteur depuis le 2026-08-10 : `agile` (ignore mobilier et figures, pas les murs), `racines_entravantes` (arrête net le héros qui entre au contact), `venimeux` (paralysie, sauf 5-6 sur un d6), `tacticien` (+1 dé contre une cible flanquée, puis décrochage), `spawn` (engendre la créature nommée par la capacité) et `ethere` (traverse tout, mais ne s'arrête ni sur une case occupée ni dans une salle non découverte — et **seul un bouclier noir le touche**, sauf sort ou artefact). Porter le venin a obligé à câbler `deplacement_interdit`, une clé de condition qui n'avait **aucun lecteur** : un héros « Immobilisé » marchait normalement.
...
Suite **Pest** (moteur sous `tests/Unit/Engine`, jeu sous `tests/Feature`) — **632 tests verts**.
