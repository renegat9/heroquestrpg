/*
 * Capture la scène de table DU DÉ DE DÉPLACEMENT (le complément à un seul
 * genre de livret-scenes.mjs, même patron). À lancer AVANT
 * livret-scene-deplacement.php.
 *
 *   node browser-shots/livret-scene-deplacement.mjs <code>
 */
import { chromium } from 'playwright';

const code = process.argv[2];
if (!code) { console.error('usage : node browser-shots/livret-scene-deplacement.mjs <code>'); process.exit(1); }
...
for (let i = 0; i < 700 && !prise; i++) {
    if (await p.locator('.scn-carte').count()) {
        ...
        if (/^Au tour de Grom$/.test(titre)) {
            prise = true;
            await p.waitForTimeout(700);
            const r = await p.locator('.scn-carte').boundingBox();
            const m = 14;
            await p.screenshot({ path: '/work/browser-shots/livret/77-scene-deplacement.png', clip: {
                x: Math.max(0, r.x - m), y: Math.max(0, r.y - m),
                width: r.width + 2 * m, height: r.height + 2 * m } });
            console.log('77-scene-deplacement · ' + titre);
        }
    }
    await p.waitForTimeout(200);
}
console.log(prise ? 'PRISE' : 'MANQUANTE : 77-scene-deplacement');
await b.close();
