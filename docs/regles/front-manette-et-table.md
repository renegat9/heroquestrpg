...
⚠ **Traverser les figures n'est PAS être Agile** (René, 2026-09-17 : « on se retrouve parfois à se déplacer à travers les mobiliers »). La décision était bien unique, mais les deux sites qui l'**appliquent** (`ResolveurTour::resoudreDeplacement()`, `MenuMoteur::peutSeDeplacer()`) appelaient `Grille::autoriserFranchissement()` — le mode **Agile** des monstres (Jungles of Delthrak : « ignore le terrain gênant, le mobilier et les héros »), qui vide aussi `$obstacles`. Le Rogue et tout porteur de `franchit_figures` (Voile de Brume, Poudre d'Invisibilité, Cape des Ombres) traversaient donc tables et armoires. « Parfois » parce que seul le **plus court** chemin passait par le meuble, et que la manette — qui, elle, bloquait le mobilier — n'offrait que des destinations atteignables en le contournant : le serveur choisissait ensuite un chemin **que le joueur n'avait pas vu**. La carte dit « cases occupées par des monstres » : c'est `autoriserFranchissementFigures()` (figures seules), le mode du Patinage. `MobiliteCombatRogueTest` le verrouille en jeu — une table dans un couloir forcé, et un Rogue cerné de tables à qui le menu ne promet plus « Se déplacer ».

⚠ **Le client ne calcule pas le TRAJET non plus** (2026-09-17). `DeplacementSheet` garde son BFS de cases atteignables — un miroir assumé, revalidé par le serveur — mais le **chemin** vient de `POST deplacement/apercu` : deux routes de même coût n'exposent pas aux mêmes pièges (`controlerChemin()` contrôle case par case), donc un chemin re-dérivé en JS annoncerait un trajet que le héros ne prendra pas. Premier tap = le trajet s'affiche (coût, points restants, pièges CONNUS traversés) ; second tap ou bouton « Y aller » = il part. ⚠ Un échec de l'aperçu (réseau, 422) ne bloque JAMAIS le déplacement : la visée reste, le second tap part comme avant — un confort ne doit pas pouvoir geler un tour. ⚠ Les murs de glace (`carte.glace`) entrent dans le même jeu de cases bloquantes que le mobilier côté manette, comme dans `$obstacles` côté serveur.
...
⚠ **La table annonce chaque début de tour : portrait, dé ROUGE, calcul** (René, 2026-09-16 — genre `deplacement` de `.table.scene`). La scène part de `MenuMoteur` **à l'instant du lancer**, le seul où les dés réels existent : `deplacement_tour` ne garde que le total, et l'option `se_deplacer` reconstitue encore le dé par `total − base`, faux dès qu'un malus d'armure, des Raquettes ou un second dé s'en mêlent. Le `calcul` est une phrase **décidée par le serveur**, chaque nombre nommé (« base 4 + dé 6 = 10 » — « 3 + 5 = 8 » ne disait pas lequel était le dé, vu sur la première capture), et sa portée sort de `MenuMoteur::porteeDuTour()`, le même calcul que l'option du téléphone : vérifié en partie réelle, la table et la manette annoncent les mêmes cases. Titre élidé devant une voyelle (« Au tour d'Aldric »). ⚠ **L'écran de table enchaîne ses scènes dans une FILE** (8 au plus, les plus récentes gardées) : l'ancienne place d'attente UNIQUE était écrasée par chaque nouvelle scène — trois monstres qui frappent n'en montraient que deux, et une chute suivie d'un début de tour aurait perdu la chute, chaque tour. ⚠ Et une scène qui arrive sous la carte d'ouverture ou le prologue **attend** leur fermeture : elle devenait « courante » dessous et son minuteur s'écoulait sans témoin — le premier jet d'une quête partait toujours dans le vide.

⚠ **Les monstres marchent sur la table comme les héros — et les réactions des manettes s'y voient** (René, 2026-09-17 : « on avait constaté que le mouvement des monstres n'était pas animé comme ceux des joueurs » ; « ne pas oublier qu'une manette peut aussi réagir durant le tour d'un monstre »). Le constat, **mesuré** image par image sur la vraie stack, a écarté l'hypothèse de départ : le serveur envoyait bien les trajets des monstres, et un monstre seul marchait exactement comme un héros. Le défaut ne se voyait qu'**à partir de deux** : les trajets se jouent l'un après l'autre, l'état posé aussitôt envoyait les suivants à l'arrivée, où ils restaient le temps de la marche précédente avant de revenir d'un coup au départ et de refaire le chemin (33,19 à 7 900 ms, retour en 37,19 à 8 756 ms). Quatre corrections, chacune pour une faute mesurée :
- **Toutes les figurines d'un lot sont tenues sur leur départ dès réception** (`TableView::jouerMouvements()`), pas seulement celle qui marche.
- **Les trajets voyagent DANS l'état** (`EtatGroupeDiffuse::$mouvements`) — `.mouvement.anime` a disparu. La file `temps-reel` a **deux** workers : un message séparé « juste avant » n'avait aucune garantie d'ordre.
- **Une scène attend la fin de la marche de sa figurine** (`figure`, publiée seulement si elle a marché dans la même résolution — `ResolveurTour::figuresEnMarche()`). « Gardien des Cryptes attaque Borin » s'affichait au centre de la carte pendant que le monstre marchait encore. ⚠ Et la table attend ce trajet **même s'il n'est pas encore arrivé** (3 s au plus) : la scène, petit message, précède couramment l'état, gros message publié par l'autre worker — **756 ms d'avance mesurées**. Une marge fixe de 250 ms essayée d'abord ne tenait que parce qu'une autre popup occupait l'écran ; le cas inverse a été rejoué en injectant scène puis état sur le vrai canal. La tête de file bloque les suivantes : l'ordre reste celui du jeu.
- **La caméra ne quitte pas les monstres pendant qu'ils marchent** : la phase des monstres rouvre le round dans la même requête, et l'état qui porte leurs trajets désignait déjà le héros suivant.

⚠ **La phase des monstres ne s'arrête PAS pendant qu'un joueur réfléchit à sa réaction** (`MoteurReactions`) : l'offre part sur `joueur.{id}` à l'instant du coup, avec son compte à rebours, et rien de ce qui précède ne la retarde. La réaction acceptée arrive donc **après**, par sa propre route — et elle n'avait **aucune scène** : la table montrait l'attaque, jamais la parade. `ReactionController` émet maintenant les scènes de `SceneDeTable::depuisReaction()` (genre `reaction`, ou la frappe elle-même pour *Représailles*), dont le texte est celui du fil de combat (`JournalCombat::issueReaction()`), puis vide le tampon pour qu'une relève suive sa scène. Vérifié en partie réelle : *Cendres du Phénix* acceptées depuis la manette, scène sur la table.
