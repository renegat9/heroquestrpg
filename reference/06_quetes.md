...
8. **Exploration — fouille (Phase 2, doc 14)** : **deux actions distinctes**. *Fouiller la zone* = un seul jet de Mind (diff 1) qui révèle pièges ET portes secrètes dans le rayon. *Fouiller — trésor* = action séparée à risque, offerte dans une salle vide non encore fouillée : on **pioche une carte** dans le **deck de fouille** de la quête (`structure.deck_fouille` : or / potion / piège / monstre errant / rien), bâti au démarrage et pioché **sans remise** — la composition est donc garantie, contrairement à l'ancienne table pondérée `tresor_a_risque`. Le **monstre errant** ne sort que par *Fouiller — trésor* (jamais par *Fouiller la zone*) et est décompté d'un **budget errant dédié** (`quetes.budget_errant`, semé depuis `structure.budget_errant`), distinct du budget de rencontre. **Question ouverte tranchée** : le budget errant est une colonne de `quetes`, persistée et restaurée par les snapshots — plus aucun état de jeu durable ne vit en cache.
9. **Artefacts (armes uniques)** : chaque quête désigne **une** salle-coffre — la plus profonde dans l'arbre des couloirs — abritant **au plus une** arme de rareté `unique`. Elle **ne consomme aucune carte** du deck (bonus net) et le héros qui la fouille la reçoit, ce qui règle « à qui va l'objet » sans rituel de partage. Aucune arme disponible (toutes déjà détenues par le groupe, unicité **par groupe**) → le coffre verse `deck_fouille.or_coffre`. Ni achat, ni revente, ni Forge (doc 04 §4).
10. **Portes à restriction (Phase 2, doc 14)** : verrous `cle` (objet en inventaire, action *Ouvrir la porte* au contact), `monstres_vaincus` (ouverture auto post-combat), `levier` (action *Actionner le levier* au contact). Une porte non ouverte est infranchissable + opaque ; *Traverser la Pierre* franchit déjà les murs. Verrou `jet`-pour-forcer reporté.
